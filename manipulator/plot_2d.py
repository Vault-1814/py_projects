...
def draw2d(l, theta, q, iq=(1, 2, 4), point=[0, 0, 0], point_gripper=[0, 0, 0]):
    subplot = 221
    qty_points = len(iq) + 1
    for k in range(0, len(q)):
        ...
        for i in range(1, qty_points):
            sum_angles += q[k, iq[i - 1]] #+ theta[iq[i - 1]]
...
def draw1d(l, theta, q, iq=(1, 2, 4), point=[0, 0, 0], point_gripper=[0, 0, 0]):
    qty_points = len(iq) + 1
    x = np.zeros(qty_points)
    y = np.zeros(qty_points)
    # i am indian *_+
    sum_angles = 0
    for i in range(1, qty_points):
        sum_angles += q[iq[i-1]] #+ theta[iq[i-1]]
        x[i], y[i] = f(l[iq[i-1]+1], sum_angles, x[i-1], y[i-1])
